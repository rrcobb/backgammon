...
import type { Factors } from "../strategy/factors";
...
const initialFactors: Factors[] = JSON.parse(await Bun.file(path).text());

// Generate synthetic factors to reach target population size
function generateSynthetics(basePopulation: Factors[], targetSize: number): Factors[] {
...
function tournamentSelect(population: Factors[], fitnesses: number[], tournamentSize: number): Factors {
  const indices = Array.from({ length: population.length }, (_, i) => i);
  const shuffled = indices.sort(() => Math.random() - 0.5);
  const candidates = shuffled.slice(0, tournamentSize).map((i) => ({ index: i, fitness: fitnesses[i] }));

  const winner = candidates.reduce((best, current) => (current.fitness > best.fitness ? current : best));
