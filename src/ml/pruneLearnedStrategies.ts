...
async function tournamentAndPrune(
  ...
  for (const group of similarGroups) {
    ...
    if (groupInfos.length > 1) {
      ...
      console.log(`Pruned similar group, kept learned${groupInfos[0].index}, removed:`,
        groupInfos.slice(1, maxToRemove + 1).map(info => `learned${info.index}`).join(', ')
      );
    }
  }
  ...
  if (remainingCount > targetSize) {
    ...
    const sortedRemaining = strategyInfos
      .filter(info => info.keep)
      .sort((a, b) => a.winRate - b.winRate); // Sort ascending for worst first
    ...
    console.log('Removed worst performers:', 
      sortedRemaining
